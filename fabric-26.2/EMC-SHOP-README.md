...
This is a focused prototype rather than a full ProjectE port. `data/projecte/emc_values.json` contains 748 vanilla EMC prices generated from ProjectEF's explicit defaults, selected seed values, and Minecraft 26.2 recipes. Run `tools/generate_emc_values.py` with a 26.2 server jar to regenerate the fixed registry. Both client and server read the same file. Unvalued items are not sold or learned; hover them for a clear message. Durable equipment and component-dependent items remain unvalued because the current shop tracks item IDs rather than component-aware item variants. Selling consumes the selected inventory stack up to 64; purchases produce one item (right-click requests up to 64).
